...
## 待解决问题
* [ ] 目前只是简单下载，没有考虑到并发和效率，下载速度较慢，目前在局域网内测设没有问题
* [ ] 下载已经下载好的文件，不会重复下载(根据文档名称或者其他唯一标志符号)
* [ ] 支持表格下载，目前文档中的表格在网页源码中排列混乱，同时还需要结合CSS来进行布局，后续会想别的方法．
* [ ] 支持excel表格下载，目前还没有尝试，后续会试一试 ．
# 有建议可以提，我会挑有价值的建议来实现的
## 环境安装
#### web框架主要用到了django3，部署使用uvicorn，库看requirements.txt,在使用时缺少啥安装啥也可以。
...
## 使用方式：
#### 1.把example_conf.py文件中的ip和端口改成你自己的
#### 2.运行start_example.py
#### 3.访问你的http://192.168.0.102:8000/bdwk/download_doc/ , 记住把ip和端口换成你自己的
#### 4.粘贴地址，点击下载，下载时间可能较长，请耐心等待
