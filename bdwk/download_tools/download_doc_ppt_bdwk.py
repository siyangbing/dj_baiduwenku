...
chromedriver_path = os.path.join(BASE_DIR, "bdwk/download_tools/chromedriver")
# chromedriver_path = "/home/db/PycharmProjects/dj_baiduwenku/"

doc_dir_path = os.path.join(BASE_DIR, "download_files/doc")
ppt_dir_path = os.path.join(BASE_DIR, "download_files/ppt")
#
# doc_dir_path = "./doc"
# ppt_dir_path = "./ppt"
...
class StartChrome():
    ...
    def create_ppt_doc(self, ppt_dir_path=ppt_dir_path, doc_dir_path=doc_dir_path):
        # 点击关闭开通会员按钮
        ...
        if contents:  # 如果是ppt
            file_path = self.create_ppt(ppt_dir_path, sel)
        else:  # 如果是doc
            file_path = self.create_doc(doc_dir_path, sel)
        # a = 3333
        self.brower.quit()
        return file_path
    ...
    def create_ppt(self, ppt_dir_path, sel):
        ...
        ppt_download_file_path = os.path.join(ppt_dir_path, title + ".pptx")
        # print("-1 {}".format(ppt_download_file_path))
        print("download1 {} success!".format(ppt_download_file_path))
        return ppt_download_file_path
    ...
    def create_doc(self, doc_dir_path, sel):
        ...
        if xpath_content_one.endswith("text()"):  # 如果是文字就直接爬
            ...
            docx_download_file_path = os.path.join(doc_dir_path, '{}.docx'.format(title))
            # print("-2 {}".format(docx_download_file_path))
            document.save(docx_download_file_path)
            print("download2 {} success!".format(title))
            ...
            docx_download_file_path = os.path.join(doc_dir_path, '{}.docx'.format(title))
            # print("-3 {}".format(docx_download_file_path))
            document.save(docx_download_file_path)  # 保存文档到指定位置
            print("download3 {} success!".format(title))
        return docx_download_file_path


if __name__ == "__main__":
    start_chrome = StartChrome(url)
    path = start_chrome.create_ppt_doc()
